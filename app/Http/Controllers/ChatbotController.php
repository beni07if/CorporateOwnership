...
class ChatbotController extends Controller
{
    ...
    {
        $input = $request->input('message'); // ambil input pesan dari users
        ...
        if ($subsidiaries->isNotEmpty()) {
            ...
            if (count($shareholder_data) > 1) {
                if ($total_share > 50) {
                    $response = $subsidiary->subsidiary . ' adalah anak perusahaan dari group ' . $subsidiary->group_name . ' yang berlokasi di Kabupaten ' . implode(', Kabupaten ', $regencies0) . ', Provinsi ' . implode(', Provinsi ', $provinces0) . ', ' . implode(', ', $countries0) . '. Aktivitas utama ' .  $subsidiary->subsidiary . ' adalah ' . implode(' dan ', $subsidiaries->pluck('principal_activities')->unique()->toArray()) . '. Mayoritas kepemilikan sahamnya dimiliki oleh <a href="#">' . $majority_shareholder . '</a> sebesar ' . $majority_share_percentage . '% dan sisanya dimiliki oleh ' . implode(', ', array_map(function ($data) {
                        return '<a href="#">' . $data['name'] . '</a> (' . $data['share_percentage'] . '%)';
                    }, array_slice($shareholder_data, 1))) . '. ';
                } else {
                    $response = $subsidiary->subsidiary . ' adalah anak perusahaan dari group ' . $subsidiary->group_name . ' yang berlokasi di Kabupaten ' . implode(', Kabupaten ', $regencies0) . ', Provinsi ' . implode(', Provinsi ', $provinces0) . ', ' . implode(', ', $countries0) . '. Aktivitas utama ' .  $subsidiary->subsidiary . ' adalah ' . implode(' dan ', $subsidiaries->pluck('principal_activities')->unique()->toArray()) . '. Kepemilikan sahamnya didistribusikan di antara beberapa pemegang saham, yaitu ' . implode(', ', array_map(function ($data) {
                        return '<a href="#">' . $data['name'] . '</a> sebesar ' . $data['share_percentage'] . '%';
                    }, $shareholder_data)) . '. ';
                }
            } else {
                $response = $subsidiary->subsidiary . ' adalah anak perusahaan dari group ' . $subsidiary->group_name . ' yang berlokasi di Kabupaten ' . implode(', Kabupaten ', $regencies0) . ', Provinsi ' . implode(', Provinsi ', $provinces0) . ', ' . implode(', ', $countries0) . '. Aktivitas utama ' .  $subsidiary->subsidiary . ' adalah ' . implode(' dan ', $subsidiaries->pluck('principal_activities')->unique()->toArray()) . '. Kepemilikan sahamnya dimiliki oleh ' . implode(', ', array_map(
                    function ($data) {
                        return '<a href="#">' . $data['name'] . '</a> sebesar ' . $data['share_percentage'] . '%';
                    },
                    $shareholder_data
